...
import { StreamConfig } from '../../utils/livestream';

interface StreamConfigSidebarProps {
    config: StreamConfig;
    onConfigChange: (updated: Partial<StreamConfig>) => void;
}

export const StreamConfigSidebar: React.FC<StreamConfigSidebarProps> = ({ config, onConfigChange }) => {
    return (
            ...
            <div className="bg-surface dark:bg-surface-dark rounded-xl border border-border dark:border-border-dark p-5 shadow-sm h-full">
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-1 bg-surface-hover dark:bg-surface-dark-lighter p-1 rounded-lg">
                        <button
                            onClick={() => onConfigChange({ videoOrder: 'loop' })}
                        ...
                        <button
                            onClick={() => onConfigChange({ videoOrder: 'shuffle' })}
                            className={`py-2 px-3 text-sm font-medium rounded-md transition-colors flex items-center justify-center gap-2 ${
                ...
                <div className="mb-8">
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-1 bg-surface-hover dark:bg-surface-dark-lighter p-1 rounded-lg">
                                <button
                                    onClick={() => onConfigChange({ protocol: 'rtmp' })}
                                ...
                                <button
                                    onClick={() => onConfigChange({ protocol: 'hls' })}
                                    className={`py-2 px-3 text-sm font-medium rounded-md transition-colors ${
                                        config.protocol === 'hls'
                                            ? 'bg-primary text-white shadow-sm'
                                            : 'text-text-secondary hover:text-text-primary'
                        ...
                        <label className="flex items-center gap-3 text-sm text-text-primary cursor-pointer hover:text-primary transition-colors">
                            <input
                                ...
                                onChange={(e) => onConfigChange({ autoStop: e.target.checked })}
                            />
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center gap-3 text-sm text-text-primary cursor-pointer hover:text-primary transition-colors">
                            <input
                                ...
                                onChange={(e) => onConfigChange({ scheduleStart: e.target.checked })}
                            />
                            Schedule start time
                        </label>
                        {config.scheduleStart && (
                            <input
                                ...
                                onChange={(e) => onConfigChange({ scheduledStartTime: e.target.value })}
                            />
                        )}
                        <label className="flex items-center gap-3 text-sm text-text-primary cursor-pointer hover:text-primary transition-colors">
                            <input
                                ...
                                onChange={(e) => onConfigChange({ scheduleEnd: e.target.checked })}
                            />
                            Schedule end time
                        </label>
                        {config.scheduleEnd && (
                            <input
                                ...
                                onChange={(e) => onConfigChange({ scheduledEndTime: e.target.value })}
                            />
                        )}
                    </div>
                </div>
...
export default StreamConfigSidebar;
