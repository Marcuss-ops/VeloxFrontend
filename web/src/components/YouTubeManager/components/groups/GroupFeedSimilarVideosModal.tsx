...
import { addToStudio, copyToClipboard } from '../../../../lib/utils';
...
interface GroupFeedSimilarVideosModalProps {
    videos: SimilarVideo[];
    loading: boolean;
    groupName: string;
    onClose: () => void;
...
export const GroupFeedSimilarVideosModal: React.FC<GroupFeedSimilarVideosModalProps> = ({
    videos,
    loading,
    groupName,
    onClose,
    ...
    return (
        ...
        >
            <div
                className="bg-slate-900 rounded-2xl w-full max-w-7xl max-h-[95vh] overflow-hidden border border-purple-500/30 shadow-2xl flex flex-col"
                            ...
                            {videos.map((v, i) => (
                                <div key={i} className="bg-slate-800/50 rounded-lg overflow-hidden hover:bg-slate-800 transition-colors group">
                                    <div className="relative aspect-video bg-black">
                                        {v.thumbnail && (
                                            <img src={v.thumbnail} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="" />
                                        )}
                                        ...
                                        <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                                            <button
                                                onClick={() => addToStudio(v, groupName)}
                                                className="bg-black/80 hover:bg-emerald-600 p-1 rounded transition-all"
                                                title="Aggiungi a Creator Studio"
                                            >
                                                <span className="material-symbols-rounded text-[14px] text-white">edit</span>
                                            </button>
                                            <button
                                                onClick={() => copyToClipboard(v.url || '')}
                                                className="bg-black/80 hover:bg-purple-600 p-1 rounded transition-all"
                                                title="Copia link"
                                            >
