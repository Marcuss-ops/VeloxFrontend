...
interface HomeViewNewsModalProps {
    news: NewsResult;
    content: string;
    loading: boolean;
    youtubeVideos: { title: string; url: string; thumbnail?: string; views?: string }[];
...
export const HomeViewNewsModal: React.FC<HomeViewNewsModalProps> = ({
    news,
    content,
    loading,
    youtubeVideos,
            ...
            <div className="bg-slate-900 rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-purple-500/30 shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-auto p-4 space-y-6">
                    <div className="bg-slate-800/50 rounded-xl p-4">
                        ...
                        {loading ? <div className="flex items-center gap-2 text-gray-500"><span className="material-symbols-rounded animate-spin">progress_activity</span> Caricamento...</div> : <p className="text-sm text-gray-300 leading-relaxed">{content || 'Contenuto non disponibile'}</p>}
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            <div><label className="text-[10px] text-gray-500 uppercase">Idea Hook</label><input value={notes.hook} onChange={e => onNoteChange('hook', e.target.value)} placeholder="Hook iniziale..." className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-purple-500" /></div>
                            <div><label className="text-[10px] text-gray-500 uppercase">Taglio del Video</label><input value={notes.cut} onChange={e => onNoteChange('cut', e.target.value)} placeholder="Come strutturare..." className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-purple-500" /></div>
                            <div><label className="text-[10px] text-gray-500 uppercase">Thumbnail Idea</label><input value={notes.thumbnail} onChange={e => onNoteChange('thumbnail', e.target.value)} placeholder="Idea thumbnail..." className="w-full mt-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-purple-500" /></div>
                        </div>
                    </div>
                </div>
                <div className="p-4 border-t border-white/10 bg-slate-800/50">
                    <div className="flex flex-wrap gap-2">
                        {(['watchlist', 'todo', 'ignored', 'archived'] as const).map(s => (
                            <button key={s} onClick={() => onStatusChange(s)} className={`flex-1 min-w-[100px] px-3 py-2 rounded-lg text-sm font-medium transition-all flex items-center justify-center gap-1 ${status === s ? (s === 'watchlist' ? 'bg-blue-600' : s === 'todo' ? 'bg-green-600' : s === 'ignored' ? 'bg-gray-600' : 'bg-purple-600') + ' text-white' : 'bg-slate-700 text-gray-300 hover:bg-slate-600'}`}>
                                <span className="material-symbols-rounded text-[14px]">{s === 'watchlist' ? 'bookmark' : s === 'todo' ? 'checklist' : s === 'ignored' ? 'block' : 'archive'}</span>
