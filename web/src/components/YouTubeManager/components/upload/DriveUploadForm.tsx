...
import type { FileItem, ChannelGroup } from '../../utils/driveImporter';
...
interface DriveUploadFormProps {
    ...
    onVisibilityChange: (value: 'private' | 'public' | 'unlisted') => void;
    onScheduleDateChange: (value: string) => void;
    onScheduleTimeChange: (value: string) => void;
    onThumbnailModeChange: (mode: 'none' | 'drive') => void;
    onGroupSelect: (groupId: string, channelIds: string[]) => void;
    onToggleChannel: (channelId: string) => void;
    onPublishNow: () => void;
    onSchedule: () => void;
}

export const DriveUploadForm: React.FC<DriveUploadFormProps> = ({
    selectedFiles,
    selectedIds,
    title,
    description,
    ...
    return (
        <div className={`bg-surface dark:bg-surface-dark rounded-xl border border-border dark:border-border-dark p-4 space-y-4 transition-all ${!hasSelection ? 'opacity-60' : ''}`}>
            ...
            <div className="space-y-1.5">
                ...
                <div className="grid gap-3 md:grid-cols-3">
                    {([
                        { value: 'private' as const, label: 'Private', helper: 'Visible only to the channel owner.' },
                        { value: 'public' as const, label: 'Public', helper: 'Published and visible to everyone.' },
                        { value: 'unlisted' as const, label: 'Unlisted', helper: 'Accessible only through a direct link.' },
                    ]).map((option) => (
                        <button
                            key={option.value}
            ...
            <div className="space-y-3 pt-2 border-t border-gray-700">
                ...
                <div className="flex gap-2">
                    <button
                        onClick={() => onThumbnailModeChange('none')}
                    ...
                    <button
                        onClick={() => onThumbnailModeChange('drive')}
                        className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium transition-all ${
                            thumbnailMode === 'drive'
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {channelGroups.map(group => (
                                <button
                                    key={group.id}
                                    onClick={() => onGroupSelect(group.id, group.channels.map(ch => ch.id))}
            ...
            <div className="flex gap-3 pt-2">
                <button
                    ...
                    disabled={isPublishing || !title.trim() || !hasSelection}
                ...
                <button
                    onClick={onSchedule}
                    disabled={isPublishing || !title.trim() || !scheduleDate || !scheduleTime || !hasSelection}
