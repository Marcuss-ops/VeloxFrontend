        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            {data.loading ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[0, 1, 2].map(i => (
                        <div key={i} className="animate-pulse rounded-xl border border bg-card/80 p-5">
                ...
                <>
                    {/* Summary Cards */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="rounded-xl border border bg-card/80 p-5 backdrop-blur-xl">
                            <div className="flex items-center gap-3 mb-3">
                                <div className="size-10 rounded-lg bg-blue-500/10 flex items-center justify-center">
                                    <Eye className="size-5 text-blue-400" />
                                </div>
                                <span className="text-muted-foreground text-sm">Visualizzazioni Totali</span>
                            </div>
                            <div className="text-3xl font-bold text-white">
                                {formatNumber(viewsInRange)}
                            </div>
                            <div className="mt-2 text-xs text-muted-foreground">Ultimi {period} giorni</div>
                        </div>
                        <div className="rounded-xl border border bg-card/80 p-5 backdrop-blur-xl">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-muted-foreground text-sm">Revenue Totale</span>
                            </div>
                        ...
                        <div className="rounded-xl border border bg-card/80 p-5 backdrop-blur-xl">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-muted-foreground text-sm">Copertura Dashboard</span>
                            </div>
                            <div className="text-3xl font-bold text-white">
                                {daysVisible}
                            </div>
                            <div className="mt-2 text-xs text-muted-foreground">{channelsVisible} canali mostrati</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {/* Views Chart */}
                        <div className="rounded-xl border border bg-card/80 backdrop-blur-xl rounded-xl p-5">
                    ...
                    <div className="rounded-xl border border bg-card/80 backdrop-blur-xl rounded-xl p-5">
                        ...
                        <div className="space-y-3">
                            {data.topChannels.length === 0 ? (
                                <p className="text-muted-foreground text-sm text-center py-4">Nessun dato</p>
                            ) : (
                                data.topChannels.map((channel, i) => (
                                    <div key={channel.channel_id || i} className="flex items-center gap-3 p-2 rounded-lg bg-card/50 hover:bg-card transition-colors">
                                        ...
                                        {channel.thumbnail_url ? (
                                            ...
                                            <div className="size-12 rounded-full bg-card flex items-center justify-center">
