...
export const DrivePickerHeader: React.FC<DrivePickerHeaderProps> = ({
    ...
    return (
        <>
            ...
            <div className="px-5 py-3 border-b border flex flex-wrap items-center gap-2">
                {path.map((p, i) => (
                    <button
                        key={`${p.id}-${i}`}
                        onClick={() => loadFolder(p.id, path.slice(0, i + 1))}
                        className="text-[11px] px-2 py-1 rounded bg-card/70 border border text-foreground/70 hover:text-white"
                    >
                        {p.name}
                    </button>
                ))}
            ...
            {/* Search + filters */}
            <div className="px-5 py-3 border-b border grid grid-cols-1 md:grid-cols-4 gap-2">
                <input
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    placeholder="Ricerca live file/cartelle..."
                    className="md:col-span-2 bg-background/60 border border rounded-lg px-3 py-2 text-xs text-foreground/80 placeholder:text-muted-foreground outline-none focus:border-sky-500/40"
                />
                <select
                    value={fileTypeFilter}
                    onChange={(e) => setFileTypeFilter(e.target.value as 'all' | 'video' | 'txt')}
                    className="bg-background/60 border border rounded-lg px-3 py-2 text-xs text-foreground/80 outline-none focus:border-sky-500/40"
                >
