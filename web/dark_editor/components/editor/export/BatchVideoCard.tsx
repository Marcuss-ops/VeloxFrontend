...
export function BatchVideoCard({ video, selected, previewUrl, variant, localizedMetadata, result, onToggle, onEdit }: BatchVideoCardProps) {
  ...
  return (
    <article className={`publish-card group relative min-w-0 overflow-hidden text-left transition-all duration-150 ${selected ? 'border-white/60 ring-1 ring-white/25' : 'hover:border-white/[0.14] hover:bg-white/[0.03]'}`} data-video-key={videoKey(video)}>
      <div
        role="button"
        tabIndex={0}
        ...
        className="relative aspect-video cursor-pointer overflow-hidden bg-[#24243f] outline-none focus-visible:ring-2 focus-visible:ring-white/50"
      >
        ...
        <span className={`absolute left-2.5 top-2.5 grid h-5 w-5 place-items-center rounded-full border text-[11px] ${selected ? 'border-white bg-white text-black' : 'border-white/20 bg-black/40 text-transparent'}`}>✓</span>
      ...
      <div
        ...
        className={`space-y-1 p-3 ${onEdit ? 'cursor-pointer outline-none hover:bg-white/[0.035] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/50' : ''}`}
